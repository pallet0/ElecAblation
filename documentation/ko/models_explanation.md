...
이 문서는 EEG 제거 연구(Ablation Study)에 사용된 딥러닝 모델들을 설명합니다. 목표는 EEG 신호로부터 감정을 분류하는 것입니다. 여기서 핵심 혁신은 **어텐션 메커니즘(Attention Mechanisms)**의 사용으로, 모델이 뇌의 모든 부분을 동등하게 취급하는 대신 감정에 가장 중요한 특정 부분(채널)에 "집중(focus)"할 수 있게 합니다.

## 1. `ChannelAttentionEEGNet`

이것이 메인 모델입니다. 어떤 EEG 전극(채널)이 중요한지 학습합니다.
...
```
*   **로직:** 310개의 숫자를 모두 가져와 하나의 긴 리스트로 펼치고(flatten), 표준 신경망에 입력합니다.
*   **왜 비교하는가?** 뇌의 "구조"가 중요하다는 것을 증명하기 위함입니다. 이 모델은 "채널 1 대역 1"과 "채널 62 대역 5"를 특정 위치에서 왔다는 정보 없이 리스트의 무작위 숫자로 취급합니다.
