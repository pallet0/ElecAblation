...
*   **MixUp 증강 (MixUp Augmentation):** 학습 루프는 MixUp을 지원합니다. 이는 예제 쌍을 혼합(보간)하여 모델이 학습 포인트 사이에서 선형적으로 동작하도록 장려하는 기술입니다.
...
*   **목표:** 15명의 피험자 각각을 위한 개인화된 `MLPBaseline` 모델을 학습시킵니다.
*   **멀티 시드 앙상블 (Multi-Seed Ensemble):** 결과가 단일 무작위 초기화에 의존하지 않고 견고함을 보장하기 위해, 이제 여러 무작위 시드에 걸친 학습을 지원합니다 (`--n_seeds` 인자로 제어).
*   **데이터 분할:**
    *   **학습:** 세션 1 + 세션 2.
    *   **테스트:** 세션 3.
*   **이유:** EEG는 사람마다 크게 다릅니다. 일반적인 "모두에게 맞는" 모델은 종종 실패합니다. 우리는 15개의 별도 모델을 학습시킵니다(시드 수만큼 반복).
...
*   **목표:** 학습된 `MLPBaseline` 모델에게 묻습니다: "어떤 채널이 유용했습니까?"
*   **방법 1: 순열 중요도 (Permutation Importance, PI):**
    *   테스트 데이터를 모델에 통과시킵니다.
    *   *한 특정 채널*의 데이터를 무작위로 섞어(정보를 파괴)버립니다.
    *   정확도가 얼마나 *떨어지는지* 측정합니다. 만약 많이 떨어진다면, 그 채널은 중요했던 것입니다.
*   **방법 2: 통합 기울기 (Integrated Gradients, IG):**
    *   입력에 대한 출력의 기울기(변화율)를 살펴보는 정교한 방법입니다.
    *   예측 결과를 입력 특징들에 수학적으로 귀속시킵니다.
*   **앙상블 통합:** 중요도 점수는 모든 시드와 모든 피험자에 대해 평균을 냅니다.
*   **안정성 진단 (Stability Diagnostics):** 여러 시드가 사용된 경우, 스크립트는 서로 다른 시드 간의 **스피어만 순위 상관계수(Spearman Rank Correlation)**를 계산하여 전극 순위가 얼마나 일관된지 측정합니다.
*   **전체 순위 (Grand Ranking):** 최종 평균 중요도 점수를 사용하여 보편적으로 중요한 뇌 영역을 찾습니다.
...
*   **전략 1 (PI-유도):** 가장 중요한 상위 5개 채널(순열 중요도로 순위 매김)을 유지하고, 그 다음 상위 10개, 15개... 순으로 늘려갑니다.
*   **전략 2 (무작위):** 무작위 5개 채널, 무작위 10개... 를 유지합니다.
*   **예상:** "PI-유도" 곡선이 "무작위" 곡선보다 훨씬 빠르게 상승해야 합니다. 이는 우리의 중요도 순위가 실제로 의미 있는 신호를 찾았음을 증명합니다.

### 5단계: 시각화 (Visualization)
*   결과를 시각화하기 위해 PDF를 생성합니다:
    *   `topomap_importance.pdf` & `topomap_importance_IG.pdf`: 뇌의 핫스팟(중요 부위)을 보여주는 열 지도 (PI 및 IG 기반).
